...
uint8_t Adafruit_MCP23017::readGPIO(uint8_t b) {
  ...
  _wire->requestFrom(MCP23017_ADDRESS | i2caddr, 1); // sendstop false?
  return wirerecv(_wire);
}

/**
 * Write to a single single port, A or B.
 * @param b Decides what gpio to use. Should be 0 for GPIOA, and 1 for GPIOB.
 * @param t The byte to write to the given port.
 */
void Adafruit_MCP23017::writeGPIO(uint8_t b, uint8_t t) { 
  _wire->beginTransmission(MCP23017_ADDRESS | i2caddr);
  if (b == 0)
    wiresend(MCP23017_GPIOA, _wire);
  else {
